...
Room* Room2_Build_ForwardBay()
{
	...
	Room_AddRoomExit(room, "hallway", 3);  /* 1 = the room index this exit connects to */
										 ...
										 add items to the room */
	ItemList_AddItem(Room_GetItemList(room), ExitDoor_Build());
	ItemList_AddItem(Room_GetItemList(room), QuantumCarburetor_Build());
...
Room* Room3_BuildMainHallway()
{
	...
	Room_AddRoomExit(room, "forward bay", 2);
										 /* Items
										 add items to the room */
...
Room* Room6_Build_AirLock()
{
	...
	Room_AddRoomExit(room, "hallway", 3);
	Room_AddRoomExit(room, "lounge", 7);

	/* TODO BASIC: Add room exit shortcut for "n" */
	
	Room_AddRoomExitShortcut(room, "h", 3);
...
Room* Room10_BuildEngineRoom2()
{
	...
	room = Room_Create("as you enter the left engine room you see that the engine is in working order and only has to be turned back on\n you also notice that your tools are in the closet along the far wall\n you will need these to fix the left engine");
